...
var upcoming_frames : Array = [] # Elements are arrays of pairs of int
var timer : float = 0.0
...
func add_next_frame(phoneme_labels: Array, phoneme_timings: Array) -> void:
	...
	for _i in range(3):
		if reference_time < 0:
			...
			if j >= timings_array.size():
				result.append(-1)
				emit_signal("end_of_sequence")
		reference_time += target_delta
	return result
